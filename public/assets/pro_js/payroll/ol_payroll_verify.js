...
function show_client_pop(rfh_no,cdID,po_file,finance_file){

    $('#cl_cdID').val(cdID);
     $('#cl_rfh_no').val(rfh_no);
     $('#po_file').val(po_file);
     $('#finance_file').val(finance_file);
    $.ajax({
        url:get_mail_details_link,
        method:"POST",
        data: {
            "cdID":cdID,
        },
        ...
        success:function(data) {
            if(data != ""){
                    ...
                    $('#ex_to_mail').val(data[0].to_mail);
                    $('#ex_cc_mail').val(data[0].cc_mail);
                      $('#subject').val(data[0].subject);
                    $('#message').val(data[0].message);
                    var files = data[0].files;
                    console.log(files);
                    var arr_files = [];
                    var ar_files = files.split(",");
                    //console.log(ar_files);
                    show_preview_files(cdID,rfh_no,ar_files)
            }

            $('#add_client_type').modal("show");
        }
        });
...
var cl_type =  $('#client_type').val();
if(cl_type == "External Client"){
    var po_file = $('#po_file').val();
    var fin_file = $('#finance_file').val();
    var cdID = $('#cl_cdID').val();
         preview ="<table>";
        preview += "<tr>";
        preview += '<td><a class="" href="../po_letter/'+cdID+'/'+po_file+'"  target="_blank"><i class="bi bi-eye-fill"></i>Preview po input</a><td>';
        preview += "</tr>";
        preview += "<tr>";
        preview += '<td><a class="" href="../fn_po_attach/'+cdID+'/'+fin_file+'"  target="_blank"><i class="bi bi-eye-fill"></i>Preview performa invoice</a><td>';
        preview += "</tr>";
        preview +="</table>";
        $("#ex_files").html(preview);

$('#client_type_btn').html('Send Mail');
}
else{
    //$("#ex_mail").css("display", "none");
    $('#client_type_btn').html('Send to PO');
}
    });


    $('#client_typeForm').submit(function(e) {
        var formData = new FormData(this);
        e.preventDefault();
        var cl_type =  $('#client_type').val();
        var to_mail =  $('#ex_to_mail').val();
        var subject =  $('#subject').val();

            if(cl_type == "External Client" && to_mail == "" ){
                alert("To mail is Empty!!!");
            }
            else{
                $('#client_type_btn').prop("disabled",true);
                $('#client_type_btn').html('Processing');
                $.ajax({
                    url:client_type_update_link,
                    method:"POST",
                    data: formData,
                    cache:false,
                    contentType: false,
                    ...
                    success:function(data) {
                        $('#client_type_btn').prop("disabled",false);
                        $('#client_type_btn').html('Submit');
                        if(data.response =='success'){
        ...
        $('#save_as_btn').click(function() {
            ...
            var message =  $('#message').val();
                if(to_mail == "" ){
                    ...
                    $('#save_as_btn').prop("disabled",true);
                    $('#save_as_btn').html('Processing');
                    $.ajax({
                        url:save_as_draft_mail_link,
                        method:"POST",
                        data: {
                            "cdID":cdID,
                            "cl_type":cl_type,
...
function view_client_po(po_attach,po_no,po_value,po_val,cdID){
    var preview = '<a class="" href="../po_attachments/'+cdID+'/'+po_attach+'"  target="_blank"><i class="bi bi-eye-fill"></i> Preview Client PO </a>';
    $('#v_po_att').html(preview);
    $('#v_po_no').text(po_no);
    $('#v_po_value').text(po_value);
    $('#v_po_val').text(po_val);
    $('#view_client_po_popbtn').click();
}
...
//mailing process
$(document).on("change", "#att_files", function() {   // If you want to upload without a submit button
  //  e.preventDefault();
   // var form_data = new FormData($(this).parents('form')[0]);
     //   var file_data = $("#att_files").prop("files")[0]; // Getting the properties of file from file field
       var form_data = new FormData(); // Creating object of FormData class
       // form_data.append("att_files", file_data) // Appending parameter named file with properties of file_field to form_data
       var totalfiles = document.getElementById('att_files').files.length;
      // alert(totalfiles);
       for (var index = 0; index < totalfiles; index++) {
          form_data.append("files[]", document.getElementById('att_files').files[index]);
       }
       var cdID = $('#cl_cdID').val();
        var rfh_no = $('#cl_rfh_no').val();
        form_data.append('cdID', cdID);
        form_data.append('rfh_no', rfh_no);
       // alert("file_data");
        $.ajax({
          url: upload_mail_attach_po_link, // Upload Script
          type:'POST',
          data:form_data,
          dataType: 'json',
          cache:false,
          ...
          success: function(data) {
            if(data.response ="success"){
                console.log(data);
                show_preview_files(cdID,rfh_no,data.files);
            }
            else{
...
function show_preview_files(cdID,rfh_no,files){
   ...
   for (var index = 0; index < totalfiles; index++) {
    preview += "<tr>";
    preview += '<td><a class="" href="../mail_attach/'+files[index]+'"  target="_blank"><i class="bi bi-eye-fill"></i>'+files[index]+' </a><td>';
    preview += "</tr>";
}
...
function send_offerpop(rfh_no, cdID){
	var resp = confirm("Do you want Release Offer for this Candidate?");
if (resp == true) {


    $('#or_btn_'+cdID).text('Processing');
    $('#or_btn_'+cdID).prop('disabled', true);
    send_data_budgie(cdID);
    $.ajax({
        type: "POST",
        url: send_offer_release_link,
        ...
        success: function (data) {
            ...
            if(data.response =='success'){
                ...
                setTimeout(
                    function() {
                        $('#or_btn_'+cdID).text('Release Offer');
                        $('#or_btn_'+cdID).removeAttr('disabled');
                        get_offer_list_oat_po();
