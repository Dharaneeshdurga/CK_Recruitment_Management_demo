...
$(document).on('click', '#proofdocNextbtn', function(e) {
    ...
    document.getElementById('v-pills-action-tab').click(); // Works!
...
function get_candidate_preview_details(){
    ...
    var link = "http://216.48.177.166/budgie/public/index.php/";
//alert(cdID);
    $.ajax({
        type: "POST",
       // url: get_candidate_preview_details_link,
        url:"http://216.48.177.166/budgie/public/index.php/api/get_all_candidate_details",
        data: { "cdID":cdID, },
        success: function (data) {
            if(data.candidate_basic_details.length !=0){
                ...
                $('#cdp_email').text(data.candidate_basic_details[0].email);
                ...
                $('#poi_name').text(data.candidate_basic_details[0].proof_of_identity);
                $('#poa_name').text(data.candidate_basic_details[0].proof_of_address);
