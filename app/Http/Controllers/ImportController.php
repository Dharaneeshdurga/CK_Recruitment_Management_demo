...
class ImportController extends Controller
{
    ...
    public function process_offer_letter_release(Request $req)
    {
        ...
        if($req->input('get_emp_mode') =='HEPL'){
           ...
           // $sc_hra_pa = round($sc_basic_pa / 2); //old cal
            $sc_hra_pa = round($sc_basic_pa * 0.4); // house rent per annum
            ...
            //  $sc_special_allowance_pa = 3491;
            // $sc_special_allowance_pm = round($sc_special_allowance_pa / 12);


             // $sc_special_allowance_pm = 0;
// if($get_cs_pa < 300000){
//     $sc_special_allowance_pa = 0;
//     $sc_special_allowance_pm = 100;

// }
// else{
    $sc_special_allowance_pa =  ($get_cs_pa - $sc_basic_pa - $sc_medical_allowance_pa - $sc_conveyance_expence_pa);
    $sc_special_allowance_pm =  round($sc_special_allowance_pa / 12);
//}
...
if ($sc_monthly_gross_pm_a <= 21001)
{
   // $get_emp_esi_cont_pm = round($sc_monthly_gross_pm_a * 0.0325);
    ...
    $emp_esi_cont_pa = round($emp_esi_cont_pm * 12);
...
while ($abc_ctc_pa != $get_cs_pa)
{
   if ($abc_ctc_pa > $get_cs_pa){
     $get_sa_dif = $abc_ctc_pa - $get_cs_pa;
...
else
{
    $n23 = 0;
            ...
            $sc_special_allowance_pm = preg_replace('/[^\p{L}\p{N}\s]/u', '', $sc_special_allowance_pm);
