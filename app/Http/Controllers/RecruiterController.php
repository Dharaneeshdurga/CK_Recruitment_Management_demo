...
class RecruiterController extends Controller
{
    ...
    public function process_default_status(Request $request){
        ...
        $get_assigned_to = auth()->user();
        $assigned_to = $get_assigned_to->empID; //recruiter
        $recruiter_email= $get_assigned_to->email;
        $recruiter_name= $get_assigned_to->name;
        ...
        $can_type = $user_row[0]->candidate_type;
        $can_source = $user_row[0]->candidate_source;
        $can_name = $user_row[0]->candidate_name;
        $password= uniqid();
        ...
        return response()->json(
            [
                ...
                'candidate_type' => $can_type,
                 'candidate_source' => $can_source,
                 'recruiter_name' => $recruiter_name,
                 'recruiter_id' => $assigned_to,
                 'can_name' => $can_name
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($get_candidate_profile_result)
                ...
                ->addColumn('action',function($row){
                    ...
                    $get_depart = $this->recrepo->get_tblrfh_details($input_details);
                    $depart = $get_depart[0]->department;
                         $or_btn = '<span class="badge bg-info" title="Offer Release Action" type="button" onclick="get_offer_release_pop('."'".$row->cdID."'".','."'".$row->candidate_name."'".','."'".$row->rfh_no."'".','."'".$row->hepl_recruitment_ref_number."'".','."'".$row->created_by."'".','."'".$depart."'".')"><i class="fa fa-file"></i></span>';
                        //$or_btn = $depart;
                    }
                    else{
                        $or_btn ='';
    ...
    public function send_to_payroll(Request $req){
     //  $att_file = $req->input('proof_attach');
       // exit;
        ...
        $esi_type = $req->input('esi_type');
        $attendance_format  = $req->input('attendance_format');
        $weak_off = $req->input('weak_off');
        $payroll_status_ctc = $req->input('payroll_status_ctc');
        $vertical = $req->input('vertical');
        $onboarder = $req->input('onboarder');
        $reviewer = $req->input('reviewer');
        $primary_reporter = $req->input('primary_reporter');
        $additional_reporter  = $req->input('additional_reporter');
        ...
        if($po_type == "non_po"){
            $payroll_status = 5;  // non po details send to QC

        }
        else{
            $payroll_status = 1; //po details send to oat login
        }
        ...
        $input_details = array(
            ...
            'weak_off'=>$weak_off,
            'payroll_status_ctc'=>$payroll_status_ctc,
            'vertical'=>$vertical,
            'onboarder'=>$onboarder,
            'reviewer'=>$reviewer,
            'primary_reporter'=>$primary_reporter,
            'additional_reporter'=>$additional_reporter,
        );
        ...
        if($po_type == "non_po"){
            $input_details_ad = array(
                'empID' => '900101',  //karthik david empid
            );
        }
            else{
                $input_details_ad = array(
                    'empID' => '900093',  //oat empid
                );
        ...
        if ($request->ajax()) {
            ...
            return Datatables::of($get_candidate_profile_result)
                ...
                if($row->onboard_status == "0"){
                    $btn = '<span class="badge bg-info" title="Update Doj" type="button" onclick="update_doj('."'".$row->cdID."'".')"><i class="fa fa-file"></i></span>';

                }
                else{
                    $btn = '<button class="badge bg-info" title="Update Doj" type="button" disabled="disabled" onclick="update_doj('."'".$row->cdID."'".')"><i class="fa fa-file"></i></button>';
                ...
                ->rawColumns(['offer_release','history','c_doc_status','payroll_status','offer_letter_preview','po_type','action'])
                ->make(true);
...
public function update_can_doj(Request $req){
    $up_details = array(
        'cdID' => $req->input('cdID'),
        'onboard_status' => $req->input('can_status'),
        'or_doj' => $req->input('up_doj'),
        'status' => "Candidate Onboarded",

    );

    $can_update = $this->recrepo->update_new_doj( $up_details );
...
public function get_department_name(Request $request){
    $input_details =array(
            'rfh_no' => $request->input('rfh_no')
    );
    $get_depart = $this->recrepo->get_tblrfh_details($input_details);
    echo json_encode($get_depart);
}
}
